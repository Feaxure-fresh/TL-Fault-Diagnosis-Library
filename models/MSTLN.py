...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    alpha = torch.Tensor(np.random.uniform(0.0, 1.0, real_samples.size(0))).view(-1, 1).to(real_samples.device)

    # Get random interpolation between real and fake samples
    interpolates = (alpha * real_samples + ((1 - alpha) * fake_samples)).requires_grad_(True).to(real_samples.device)
    d_interpolates = D(interpolates)
    fake = torch.Tensor(real_samples.shape[0], 1).fill_(1.0).requires_grad_(False).to(real_samples.device)
