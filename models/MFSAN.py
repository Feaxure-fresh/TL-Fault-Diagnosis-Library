...
class MFSAN(nn.Module):
    ...
    def __init__(self, in_channel=1, num_classes=3, num_source=1):
        ...
        self.sonnet = [ADDneck(512, 256) for _ in range(num_source)]
        self.cls_fc_son = [nn.Linear(256, num_classes) for _ in range(num_source)]
        self.avgpool = nn.Sequential(nn.AdaptiveAvgPool1d(1),
                                     nn.Flatten())

    def forward(self, data_tgt, data_src=None, label_src=None, source_idx=None):
        if self.training == True:
            ...
            pred = torch.zeros((logits_tgt[0].shape))
