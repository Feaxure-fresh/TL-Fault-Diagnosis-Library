...
class train_utils(object):
    ...
    def _init_data(self):
        ...
        for source in args.source_name:
            if '_' in source:
                ...
                try:
                    Dataset = getattr(data_loader, '%s_op' % src)
                except:
        ...
        if '_' in args.target_name:
            ...
            try:
                Dataset = getattr(data_loader, '%s_op' % tgt)
            except:
                raise Exception("data name type not implemented")
                    ...
                    if phase == 'train':
                        with torch.set_grad_enabled(True):
                            ...
                            if training_mode == 0:
                                ...
                                pred, loss_0, loss_1 = self.model(target_data,
                                   self.device, [source_data], [source_labels], rec=(i<num_iter/2))
                                loss = loss_0 + tradeoff[0] * loss_1
                                epoch_loss[0] += loss_0; epoch_loss[1] += loss_1
    ...
    def train_mstln(self):
        ...
        model = MSTLN.model(1, args.num_classes, self.num_source).to(self.device)
        ...
        for epoch in range((args.max_epoch+1)):
            ...
            for i in tqdm(range(num_iter)):
                tgt_inputs, tgt_labels = utils.get_next_batch(self.dataloaders, self.iters,
                                                              'train', self.device)
                # Unknown healthy state training
                utils.unfreeze_net(model)
                utils.freeze_net(D)
                optimizer.zero_grad()
                for idx in range(self.num_source):
                    loss_unknown = 0.0
                    feat, tgt_pred = model(tgt_inputs, idx)
                    unknown_mean = tgt_pred[:, -1].sum() / args.batch_size - 0.00001
                    for j in range(args.batch_size):
                        if tgt_pred[j, -1] >= unknown_mean:
                            loss_unknown -= torch.log(tgt_pred[j, -1])
                ...
                utils.freeze_net(model)
                optimizerD.zero_grad()
                for idx, src in enumerate(args.source_name):
                    loss_adv = 0.0
                    src_inputs, src_labels = utils.get_next_batch(self.dataloaders, self.iters,
                                                                  src, self.device)
                    src_feat, _ = model(src_inputs, idx)
                    tgt_feat, _ = model(tgt_inputs, idx)
                    src_val, tgt_val = D[idx](src_feat), D[idx](tgt_feat)
                    gradient_penalty = MSTLN.compute_gradient_penalty(D[idx], src_feat, tgt_feat)
                    loss_adv = -torch.mean(src_val) + torch.mean(tgt_val) + 10*gradient_penalty
                    loss_adv.backward()
                optimizerD.step()
                ...
                for idx, src in enumerate(args.source_name):
                    ...
                    loss_k += F.nll_loss(torch.log(src_pred[:, :-1]), src_labels)
                    
                    sums = (1 - tgt_pred[:, -1]).sum()
                    for j in range(args.batch_size):
                        loss_shared -= (1 - tgt_pred[j, -1]) / sums * torch.mm(
                            tgt_pred[j, :-1].view(1, -1), torch.log(tgt_pred[j, :-1].view(-1, 1)))[0][0]
                    tgt_val = F.softmax(D[idx](tgt_feat), dim=1)
                ...
                for k in range(self.num_source - 1):
                    for j in range(k+1, self.num_source):
                        loss_da_ms += utils.MFSAN_mmd(tgt_feat_list[k], tgt_feat_list[j])
                loss_tl = loss_k + tradeoff[0] * loss_da + tradeoff[1] * loss_shared + \
                                                                tradeoff[2] * loss_da_ms
                loss_tl.backward()
                optimizer.step()
    ...
    def train_man_moe(self):
        ...
        F_p = nn.Sequential(
            Man_Moe.FeatureExtractor(input_size=1, output_size=2048, dropout=args.dropout),
            Man_Moe.MixtureOfExperts(input_size=2048, num_source=self.num_source,
                                     output_size=1024, dropout=args.dropout))
        
        C = Man_Moe.ClassifierMoE(input_size=2048, num_source=self.num_source,
                    output_size=args.num_classes, dropout=args.dropout, mode=args.C_mode)
        ...
        for epoch in range(args.max_epoch+1):
            ...
            loss_G, loss_CG = defaultdict(float), defaultdict(float)
            ...
            for i in tqdm(range(num_iter), ascii=True):
                ...
                for src in args.all_src:
                    ...
                    # D accuracy
                    if args.train_mode == 'source_combine' and (idx+1) != len(args.all_src):
                        src = 'ALL'
                ...
                for src in args.source_name:
                    ...
                    loss = loss_clf + tradeoff[0] * loss_gate + 5 * tradeoff[1] * loss_c_gate
                    loss.backward()
            ...
            logging.info('\t'.join(['%.03f' % (100.0*correct[d]/total[d])
                                                            for d in total.keys()]))
            logging.info('Gate accuracy & loss:')
            logging.info('\t'.join(['%.03f' % (100.0*gate_correct[d]/total[d])
                                                            for d in total.keys()]))
            logging.info('\t'.join(['%.04f' % (loss_G[d]/total[d])
                                                            for d in total.keys()]))
            logging.info('Tagger Gate accuracy & loss:')
            logging.info('\t'.join(['%.03f' % (100.0*c_gate_correct[d]/total[d])
                                                            for d in total.keys()]))
            logging.info('\t'.join(['%.04f' % (loss_CG[d]/total[d])
                                                            for d in total.keys()]))
    ...
    def train_cadg(self):
        ...
        F_s = CADG.FeatureExtractor()
        D = CADG.Discriminator(num_source=self.num_source, num_classes=args.num_classes,
                               dropout=args.dropout)
        C = CADG.Classifier(num_classes=args.num_classes)
    
        optimizer = self._get_optimizer([F_s, C])
        optimizerD = self._get_optimizer(D)
        self.lr_scheduler = self._get_lr_scheduler(optimizer)
        ...
        for epoch in range((args.max_epoch+1)):
            ...
            C.train()
            correct, total = defaultdict(int), defaultdict(int)
            loss_D2, loss_D3, loss_C = defaultdict(float), defaultdict(float), defaultdict(float)
            
            num_iter = len(self.iters['train'])
            for i in tqdm(range(2), ascii=True):
                for idx, src in enumerate(args.source_name):
                    inputs, labels = utils.get_next_batch(self.dataloaders, self.iters,
                                                          src, self.device)
                    # F&C training
                    utils.freeze_net(D)
                    utils.unfreeze_net(F_s)
                    ...
                    correct[src] += (pred == labels).sum().item()
                    total[src] += labels.shape[0]
                    loss_s1 = F.nll_loss(logits, labels)
                    loss_s1.backward()
                    loss_C[src] += loss_s1
                    optimizer.step()
                    ...
                    feat = F_s(inputs)
                    logits = D(feat)
                    D_labels = CADG.get_D_labels(labels, args.num_classes,
                                                 self.num_source, phase=2, src_idx=idx)
                    
                    loss_s2 = F.cross_entropy(logits, D_labels)
                    loss_s2.backward()
                    loss_D2[src] += loss_s2
                    optimizerD.step()
                    ...
                    feat = F_s(inputs)
                    logits = D(feat)
                    D_labels = CADG.get_D_labels(labels, args.num_classes, self.num_source)
                    
                    loss_s3 = F.mse_loss(F.softmax(logits, dim=1), D_labels)
                    loss_s3.backward()
                    loss_D3[src] += loss_s3
                    optimizer.step()
            ...
            logging.info('Training loss step 1:')
            logging.info('\t'.join(['%.04f' % (loss_C[d]/num_iter) for d in args.source_name]))
            
            logging.info('Training loss step 2:')
            logging.info('\t'.join(['%.04f' % (loss_D2[d]/num_iter) for d in args.source_name]))
            
            logging.info('Training loss step 2:')
            logging.info('\t'.join(['%.04f' % (loss_D3[d]/num_iter) for d in args.source_name]))
            
            epoch_acc = CADG.evaluate_acc(self.dataloaders, F_s, C, self.device)
            if epoch_acc > best_acc:
                best_acc = epoch_acc
                best_epoch = epoch
                logging.info("The best model epoch {}, val-acc {:.4f}".format(best_epoch, best_acc))
