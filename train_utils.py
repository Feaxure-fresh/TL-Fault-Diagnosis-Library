...
class train_utils(object):
    ...
    def train_cadg(self):
        ...
        D = CADG.Discriminator(num_source=args.num_source, num_classes=args.num_classes,
        ...
        for epoch in range((args.max_epoch+1)):
            ...
            C.train()
            correct, total = defaultdict(int), defaultdict(int)
            loss_D2, loss_D3, loss_C = defaultdict(float), defaultdict(float)
            ...
            for i in tqdm(range(num_iter), ascii=True):
                for idx, src in enumerate(args.source_name):
                    ...
                    D_labels = CADG.get_D_labels(labels, args.num_classes,
                                                 args.num_source, phase=2, src_idx=idx)
                    ...
                    feat = F_s(inputs)
                    logits = D(feat)
                    D_labels = CADG.get_D_labels(labels, args.num_classes, args.num_source)
                    
                    loss_s3 = F.mse_Loss(F.softmax(logits), D_labels)
                    loss_s3.backward()
                    loss_D3[src] += loss_s3
                    optimizer.step()
