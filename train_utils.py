...
class train_utils(object):
    ...
    def train_1src_and_srccomb(self):
        ...
        if args.model_name in ['CNN', 'WDCNN']:
            training_mode = 0
            self.model = model(in_channel=1, num_classes=3)
        elif args.model_name in ['DAN', 'DANN']:
            training_mode = 1
            self.model = model(in_channel=1, num_classes=3)
        elif args.model_name in ['CDAN', 'ACDANN']:
            training_mode = 2
            self.model = model(in_channel=1, num_classes=3)
        elif args.model_name in ['ADACL']:
            training_mode = 3
            self.model = model(in_channel=1, num_classes=3, num_source=1)
    ...
    def train_man_moe(self):
        ...
        C = Man_Moe.MoE_CNN(input_size=128, num_source=self.num_source,
                            output_size=args.num_classes, mode=0)
        
        D = Man_Moe.Discriminator(input_size=64, num_source=self.num_source+1)
