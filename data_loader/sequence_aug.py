...
def train_test_split_(data_pd, test_size=0.2, num_classes=3):
    ...
    for i in range(num_classes):
        if type(test_size) == float:
            ...
            rng.shuffle(size)
            train_pd = train_pd.append(data_pd_tmp.loc[size[:int((1-test_size)*data_pd_tmp.shape[0])],
                                                       ['data', 'label']], ignore_index=True)
            val_pd = val_pd.append(data_pd_tmp.loc[size[int((1-test_size)*data_pd_tmp.shape[0]):],
                                                   ['data', 'label']], ignore_index=True)
        elif type(test_size) == list:
            assert len(test_size) == num_classes
            data_pd_tmp = data_pd[data_pd['label'] == i].reset_index(drop=True)
            train_pd = train_pd.append(data_pd_tmp.loc[:int((1-test_size[i])*data_pd_tmp.shape[0]),
                                                       ['data', 'label']], ignore_index=True)
            val_pd = val_pd.append(data_pd_tmp.loc[int((1-test_size[i])*data_pd_tmp.shape[0]):,
                                                   ['data', 'label']], ignore_index=True)
        else:
